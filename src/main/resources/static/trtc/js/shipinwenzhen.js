(function render() {
    $("#startvedioID").click(function () {
        var roomid=999;
        var userId=$("#userid").val();
        ajaxRequest("trtc/sig", {"userId":userId},function (data) {
            alert("success invoke")
            // 页面处理，显示视频流页面
            $("#video-section").show();
            $("#input-container").hide();
            $('#c_roomid').html(roomid);
            $('#c_userid').html(userId);
            var userSig=data.data.userToken;
            var sdkappid=data.data.sdkappid;
            initRTC({
                "userId": userId,
                "userSig": userSig,
                "sdkappid": sdkappid,
